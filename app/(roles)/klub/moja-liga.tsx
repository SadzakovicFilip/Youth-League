...
export default function KlubMojaLigaScreen() {
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <ThemedText>Svi klubovi u ligi u kojoj je tvoj klub. Tap za pregled tima.</ThemedText>
      ...
      {!loading && !errorMessage && grouped.length === 0 ? (
        <ThemedView style={styles.card}>
          <ThemedText>Nema kluba u ligi ili liga nije dodeljena.</ThemedText>
        </ThemedView>
      ) : null}
        ...
        <ThemedView key={g.league_id} style={{ gap: 6 }}>
          ...
          {g.clubs.map((c) => (
            <Pressable
              ...
              onPress={() => router.push(`/klub/klub/${c.club_id}`)}>
              <ThemedText type="defaultSemiBold">
                {c.club_name}
                {c.is_my_club ? '  (moj klub)' : ''}
              </ThemedText>
