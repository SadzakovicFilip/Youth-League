...
export default function DodajIgracaScreen() {
  ...
  const loadTrainerClubs = useCallback(async () => {
    const {
      data: { user },
  ...
  useEffect(() => {
    void loadTrainerClubs();
  }, [loadTrainerClubs]);

  useScreenPullRefresh(loadTrainerClubs);
  ...
  return (
    <RefreshableScrollView contentContainerStyle={styles.container}>
      ...
      <ThemedText>
        Forma kreira igraca i upisuje `club_memberships` sa `member_role =
        igrac`.
      </ThemedText>

      <ThemedText>
        Moji klubovi (trener):{" "}
        {trainerClubIds.length ? trainerClubIds.join(", ") : "-"}
      </ThemedText>
      ...
      >
        {loading ? (
          ...
          <ThemedText style={styles.buttonText}>Kreiraj igraca</ThemedText>
        )}
      </Pressable>
