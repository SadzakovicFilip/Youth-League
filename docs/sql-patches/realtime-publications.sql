...
-- 4) REPLICA IDENTITY:
--    - `matches`: filter ide po `id` (PK) → DEFAULT je dovoljno.
--    - `match_events`: pratimo samo INSERT (filter po NEW.match_id) → DEFAULT je dovoljno.
--    Ako kasnije zelimo UPDATE/DELETE evente sa OLD podacima — odkomentarisati:
